...
function recalcularTotal() {
	...
	VENTAS_text = "no";
      ...
      $(function() {
                const triggerSearch = () => loadcom2(1);
	...
	function loadcom2(page){
		...
		var per_page = getVal("per_pageCOM");
		var parametros = {
			"actioncom2": "ajaxcom2",
			"page": page,
			'query': query,
			'per_pagecom2': per_page,
			...
			$("#loadercom2").fadeIn('slow');
    $.ajax({
        url: 'DIARIO/clasescom/controlador_filtro.php', 
        type: 'POST',
        data: parametros,
						 beforeSend: function(objeto){
				$("#loadercom2").html("Cargando...").fadeIn().delay(500).fadeOut();
			  },
        success: function (data) {
            $(".datos_ajaxcom2").html(data).fadeIn('slow');
