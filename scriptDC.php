...
$.ajax({
    ...
    processData: false,
	 ...
	 beforeSend:function(){  
    $('#mensajeSOLICITUD').html('cargando'); 
    },    
   success:function(data){
	
	//$("#reset_SOLICITUD").load(location.href + " #reset_SOLICITUD");	
	$("#mensajeSOLICITUD").html("<span id='ACTUALIZADO' >"+data+"</span>").fadeIn().delay(3000).fadeOut();
		$.getScript(load(1));		
...
$(document).on('click', '.view_SOLICITUD', function(){
  //$('#dataModal').modal();
  var personal_id = $(this).attr("id");
  $.ajax({
  url:"DIARIO/VistaPreviaSOLICITUD.php",
   method:"POST",
   data:{personal_id:personal_id},
    beforeSend:function(){  
    $('#mensajeSOLICITUD').html('CARGANDO').fadeIn().delay(3000).fadeOut(); 
    },    
   success:function(data){
...
$(document).on('click', '.view_dataSOLICITUDborrar', function(){

  var borra_soli = $(this).attr("id");
  var borra_SOLICITUD = "borra_SOLICITUD";
  ...
  url:"DIARIO/controladorDC.php",
   method:"POST",
   data:{borra_soli:borra_soli,borra_SOLICITUD:borra_SOLICITUD},
...
/////////////////////SCRIPT enviar EMAIL//////
$(document).on('click', '#BUTTON_SOLICITUD', function(){
var EMAIL_SOLICITUD = $('#EMAIL_SOLICITUD').val();
...
var dataString = $("#form_emil_SOLICITUD").serialize();
...
data: dataString+{EMAIL_SOLICITUD:EMAIL_SOLICITUD},
...
//NOMBRE DEL BOTÓN
$(document).on('click', '.view_dataPAGOP', function(){
var personal_id = $(this).attr('id');
$.ajax({
url:'DIARIO/VistaPreviapagoproveedor.php',
method:'POST',
data:{personal_id:personal_id},
beforeSend:function(){
...
$("#enviarNUEVODE").click(function(){
const formData = new FormData($('#NUEVODEform')[0]);
...
$.ajax({
    ...
    processData: false,
	 ...
	 beforeSend:function(){  
    $('#mensajeNUEVODE').html('cargando'); 
    },    
   success:function(data){
	
		$("#reseteateNUEVODE").load(location.href + " #reseteateNUEVODE");	
			$("#mensajeNUEVODE").html("<span id='ACTUALIZADO' >"+data+"</span>");			
		$("#despleResetNUEVODE").load(location.href + " #despleResetNUEVODE");	
...
$(document).on('click', '.view_dataNUEVODE', function(){
  //$('#dataModal').modal();
  var personal_id = $(this).attr("id");
  $.ajax({
   url:"DIARIO/VistaPreviaNUEVODE.php",
   method:"POST",
   data:{personal_id:personal_id},
    beforeSend:function(){  
    $('#mensajeNUEVODE').html('CARGANDO'); 
...
$(document).on('click', '.view_databorraNUEVODE', function(){

  var borra_NUEVOD = $(this).attr("id");
  var BORRAREGISTRO_NUEVODE = "BORRAREGISTRO_NUEVODE";
  ...
  $.ajax({
   url: 'DIARIO/controladorDC.php',
   method:"POST",
   data:{borra_NUEVOD:borra_NUEVOD,BORRAREGISTRO_NUEVODE:BORRAREGISTRO_NUEVODE},
